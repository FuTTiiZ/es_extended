...
ESX.ShowInventory = function()
	...
	for i=1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].money > 0 then
			...
			table.insert(elements, {
				label     = ('<b>%s:</b>'):format(ESX.PlayerData.accounts[i].label),
				name 			= ESX.PlayerData.accounts[i].label,
				cat 			= 'money',
				type      = 'item_account',
	...
	table.insert(elements, {
		label = ('<b>Sygesikringskort</b>'),
		name = 'Sygesikringskort',
		cat = 'personal',
		value = 'sygesikringskort',
		canRemove = false,
		rare = false,
		usable = false,
		type = 'card',
	...
	for i=1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].count > 0 then
			...
			for k, v in ipairs(foods) do
				if v == ESX.PlayerData.inventory[i].name then
					food = true
					break
				end
			end
			if food then
				table.insert(elements, {
					label     = (':</b> <span class="amount">' .. ESX.PlayerData.inventory[i].count .. 'x</span>'),
	...
	for i=1, #Config.Weapons, 1 do
		...
		if HasPedGotWeapon(playerPed, weaponHash, false) and Config.Weapons[i].name ~= 'WEAPON_UNARMED' then
			...
			table.insert(elements, {
				label     = (':</b> <span class="amount">' .. ammo .. ' skud</span>'),
	...
	--[[ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if data.current.value == 'sygesikringskort' then
			table.insert(elements, {  label = 'Se dit sygesikringskort', action = 'sesygesikringskort', type = data.current.type, value = data.current.value })
			table.insert(elements, { label = 'Vis dit sygesikringskort', action = 'vissygesikringskort', type = data.current.type, value = data.current.value })
		end
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								if quantity <= pedAmmo and quantity >= 0 then
									...
									exports.pNotify:SendNotification({
										text = ("<h3><center>NetGaming Menu</center></h3><br><p>Du har <b>intet</b> ammunition til dette våben!</p>"),
										timeout = 5000,
										layout = "topRight",
										type = "error",
										queue = "global"
